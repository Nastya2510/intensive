...
function getItems(url){
	// Фич. запрос. Получили данные с файла и распарсили. JSON -> JavaScript
...
// Асинхронная функция сначала получает заданные, а потом запускает весь код
async function main(){
	const items = await getItems(itemsUrl);
	const itemsFromCart = await getItemsFromCart(itemsUrl);
	...
	// Проверка корзины, пустая или нет
	if (state.cart.length > 0){
		state.cart.forEach(renderItemInCart);
		checkCart();
		calculateTotalPrice();
